			...
			<message status="warning" v-if="product.total !== 0">
				...
				<template #body>
					<div>
						<span
							class="uk-text-bold"
						>Attention, le produit que vous êtes en train de modifier a déjà été commandé par des adhérents.</span> Il est donc préférable de supprimer les commandes associées à ce produit avant de le modifier (si elles ne sont pas utiles).
					</div>
				</template>
			</message>
		</div>
		<form class="uk-width-3-5@m uk-width-4-5@s uk-margin-auto uk-form-horizontal">
			<div uk-grid class="uk-child-width-1-2@l uk-grid-large uk-grid-divider">
				<div>
					...
					<br />
					<label for="price" class="uk-form-label">Prix du produit (en €)</label>
					<div class="uk-form-controls">
						<input
							type="number"
							name="price"
					...
					<div class="uk-form-controls">
						<input type="checkbox" name="display" v-model="product.display" class="uk-checkbox" />
					</div>
					<p class="uk-text-muted">
						Si cette case est cochée, le produit sera affiché sur le tableau :
						<router-link :to="{name: 'citrus_command'}">commander des agrumes</router-link>
					</p>
					<br />
					<label for="maybe_not_available" class="uk-form-label">Produit peut être indisponible</label>
					<div class="uk-form-controls">
						<input
							type="checkbox"
					...
					<p
						class="uk-text-muted"
					>Si cette case est cochée, ce produit va être considérer comme, possiblement, indisponible. Un message d’avertissement sera donc affiché afin de prévenir les adhérents.</p>
				</div>
				...
				<div>
					...
					<p class="uk-text-muted">Si le produit se vend à l’unité, le poids doit être égale à 1.</p>
					<br />
					<label for="maximum" class="uk-form-label">Quantité maximale commandable par adhérent</label>
					<div class="uk-form-controls">
						<input
							type="number"
							name="maximum"
							:class="['uk-input', {'uk-form-danger': product.maximum <= '0'}]"
							v-model="product.maximum"
							min="0"
						/>
					</div>
					<p class="uk-text-muted">
						Défini une quantité maximale, pour ce produit, commandable par adhérent. En plus de cette quantité, chaque adhérent ne peut pas commander plus de 6 caisses de produit.
						Si ce produit n’est pas vendu par caisse (il a donc un poids de 1), ce nombre est défini à 100.
					</p>
					<br />
					...
					<p
						class="uk-text-muted"
					>Ce nombre défini le pas d’augmentation du produit, ainsi, les adhérents pourront commander uniquement un multiple de ce nombre.</p>
				</div>
			</div>
