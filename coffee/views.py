    ...
    def form_valid(self, form):
        ...
        for coffee in coffees:
            if coffee.region == region:
                messages.warning(self.request, "Le nom de la région du café existe déjà, merci de donner un autre nom, puis de réessayer.")
...
class CreateOrigin(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_invalid(self, form):
        ...
        """
        messages.warning(self.request, "L'origine n'a pas pu être enregistrée, merci de vérifier que vous avez remplis tout les champs, puis réessayer.")
...
class UpdateOrigin(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def form_invalid(self, form):
        ...
        id_origin = self.kwargs.get('id_origin', None)
        messages.warning(self.request, "L'origine n'a pas pu être enregistré, merci de vérifier que vous avez remplis tout les champs, puis réessayer.")
...
def delete_origin(request, id_origin):
    ...
    if result:
        ...
        messages.warning(request, "L'origine n'a pas pu être supprimée, merci de réessayer.")
...
def delete_coffee(request, id_coffee):
    ...
    if result:
        ...
        messages.warning(request, "Le café n'a pas pu être supprmié, merci de réessayer.")
    ...
    for coffee, quantity in post.items():
        ...
        if len(metadata) != 3:
            mail_admins(
                ...
                "Une erreur est survenue lors de la commande de {} - {} avec l'adresse mail {}, le tableau qui a été passé en paramètre contient {} valeur (normalement il est sensé y en avoir 3) !".format(name, first_name, email, len(metadata))
            )
            result['retour'] = False
            return JsonResponse(result)
        ...
        if quantity != 0 and quantity != '':
            ...
            try:
                ...
                mail_admins(
                "Id inconnue.", 
                "Une erreur est survenue lors de la commande de {} - {} avec l'adresse mail {}, le café portant l'id : {} n'existe pas/ ou l'id du type qui est {}, mais il a été passé en paramètre.".format(name, first_name, email, metadata[0], metadata[2])
                )
                result['retour'] = False
                return JsonResponse(result)

            if int(metadata[1]) == 200:
                coffee_price[c] = (int(quantity), int(quantity) * c.two_hundred_gram_price)
                result[n] = [c.farm_coop, 200, s, quantity, int(quantity) * c.two_hundred_gram_price]
            elif int(metadata[1]) == 1000:
                result[n] = [c.farm_coop, 1000, s, quantity, int(quantity) * c.kilogram_price]
                coffee_price[c] = (int(quantity), int(quantity) * c.kilogram_price)
            else :
                mail_admins(
                "Quantité inconnue.", 
                "Une erreur est survenue lors de la commande de {} - {} avec l'adresse mail {}, a envoyé un café avec l'id : {} et une quantité de {}.".format(name, first_name, email, metadata[0], metadata[1])
                )
                result['retour'] = False
                return JsonResponse(result)
            n += 1
...
def create_command(request):
    ...
    for key, value in post.items():
        ...
        tab = key.split('/')
        if len(tab) != 3:
            messages.error(request, "Une erreur est survenue, merci de réssayer. Un rapport d'erreur a été envoyé automatiquement.")
            mail_admins(
                "Tableau trop long (ou trop court)", 
        ...
        if value != '' and value != 0:
            ...
            try:
                ...
                messages.error("Une erreur est survenue, merci de vérifier que vous avez bien rentré uniquement des nombres entiers et positifs.")
                return HttpResponseRedirect(reverse_lazy('table_command'))
            ...
            if id_coffee not in coffees_type.keys() or id_type not in coffees_type[id_coffee]:
                messages.error(request, "Une erreur est survenue, merci de réssayer ! <span class='uk-text-bold'>Un café commandé n'existe pas</span>.")
                mail_admins(
                    "Le café n'existe pas",
                    "Le café portant l'id {} avec le type d'id {} n'a pas été trouvé. Cette erreur est survenue lors de la commande de {} - {} avec l'adresse mail {}.".format(id_type, id_type, name, first_name, email)
                )
                return HttpResponseRedirect(reverse_lazy('table_command'))

            sort = Type.objects.get(id=id_type)
            cof = Coffee.objects.get(id=id_coffee)
            q = Quantity(coffee=cof, command=command_user, quantity=value, weight=weight, sort=sort)
            q.save()
            command_sommary += "{} ({}) ............... {} x {} = {} €\n".format(cof.farm_coop, sort.name, weight, value, q.get_price())

    
    subject = "Commande de café"
    from_email = "cclapiarre@zohomail.eu"
    command_sommary += "total ............... {} €\n".format(command_user.get_total_price())
    ...
    messages.success(request, "Votre commande a bien été enregistrée ! Pour vous déconnectez merci de cliquer <a class='uk-link' href='/acces-securiser/disconnection'>ici</a>.")
...
def new_command(request):
    ...
    if request.method == "POST":
        ...
        for coffee in command_format.values():
            ...
            if coffee[0] not in coffees_format.keys():
                ...
                return JsonResponse({'status_code':'danger',
                                     ...
                                     "puis de me contacter si vous rencontrez de nouveau cette erreur.".format(coffee[0])
                                     })
        ...
        try:
            command = CommandCoffee.objects.create(name=name, first_name=first_name, email=email, phone_number=phone_number)
            for id, com in command_format.items():
                
                c = Coffee.objects.get(farm_coop=com[0])
                t = Type.objects.get(name=com[2])
                if com[1] == '200 grammes':
                    q = Quantity.objects.create(coffee=c, command=command, quantity=int(com[3]), weight=200, sort=t)
                elif com[1] == '1 kg':
                    q = Quantity.objects.create(coffee=c, command=command, quantity=int(com[3]), weight=1000, sort=t)
            ...
            send_mail(subject, text_content, from_email, [email], html_message=html_content)
