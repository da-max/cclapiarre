...
#
# from backend.exceptions import BoxNumberException
# from backend.api.serializers import CommandSerializer, AmountSerializer, ProductSerializer, UserSerializer, UserWithPermissionsSerializer, CoffeeSerializer, CommandCoffeeSerializer
# from backend.citrus.models import Command, Amount, Product
# from backend.coffee.models import Coffee, Order, Amount as AmountCoffee, Type
# from backend.api.pagination import StandardLimitOffsetPagination
#
#
# # Created new signals
...
#
#
# @receiver(post_change_command, sender=Order.coffee.through)
...
#     def destroy(self, request, pk):
#         try:
#             command_coffee = Order.objects.get(id=pk)
#         except (ObjectDoesNotExist, Exception) as e:
#             return Response(self.error_response(error=e, action="la suppression"))
#         else:
...
#
#         try:
#             command = Order.objects.get(Q(email=personnal_data['email']) | Q(
...
#
#         post_change_command.send(
#             sender=Order.coffee.through, instance=command, status='add')
#
#         return Response({
#             'id': int(random() * 1000),
#             'status': 'success',
...
#
#         try:
#             command = Order.objects.get(id=pk)
#         except (ObjectDoesNotExist, Exception) as e:
#             return Response(self.error_response(error=e, action='la modification'))
#
#         try:
...
#
#         post_change_command.send(
#             sender=Order.coffee.through, instance=command, status='update')
#
#         return Response({
#             'id': int(random() * 1000),
#             'status': 'success',
